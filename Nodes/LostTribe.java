...
public class LostTribe extends Node {
    ...
    RSArea END_OF_MINE = new RSArea(new RSTile(3320, 9608, 0), new RSTile(3304, 9617, 0));
    ...
    private void checkRequirements(){
        if (Skills.getActualLevel(Skills.SKILLS.MINING) < 17 || Skills.getActualLevel(Skills.SKILLS.THIEVING) < 13
            ...
            FremennikTrials.LOST_TRIBE = false;
            FremennikTrials.checkScriptStatus(FremennikTrials.lostTribe);
        }
    }
    ...
    public void buyItems() {
        ...
        GEManager.buyItem(STEEL_PICKAXE, 500, 1);
        GEManager.buyItem(UNLIT_CANDLE_LANTERN, 500, 1);
        GEManager.buyItem(LOCKPICK, 500, 1);
        GEManager.buyItem(TINDERBOX, 500, 1);
        GEManager.buyItem(Constants.STAMINA_POTION[0], 10, 2);
        GEManager.buyItem(Constants.VARROCK_TELEPORT, 60, 5);
        GEManager.buyItem(Constants.FALADOR_TELEPORT, 60, 5);
        GEManager.buyItem(Constants.LUMBRIDGE_TELEPORT, 60, 12);
        GEManager.buyItem(Constants.AMULET_OF_GLORY[2], 10, 1);
    ...
    public void getItems() {
        ...
        BankManager.withdraw(5, true, Constants.VARROCK_TELEPORT);
        BankManager.withdraw(12, true, Constants.LUMBRIDGE_TELEPORT);
        BankManager.withdraw(5, true, Constants.FALADOR_TELEPORT);
        BankManager.withdraw(2, true, BankManager.STAMINA_POTION[0]);
        BankManager.close(true);
        Utilities.useItemOnItem(TINDERBOX, UNLIT_CANDLE_LANTERN);
    }

    public void startQuest() {
        FremennikTrials.stage = "Starting Quest";
        Utilities.walkToArea(START_AREA);
        Utilities.talkToNPC(SIGMUND);
    ...
    public void talkToDukeHoracio() {
        FremennikTrials.stage = "Talking to Duke Horacio";
        Utilities.walkToArea(START_AREA);
        Utilities.talkToNPC(DUKE_HORACIO);
        NPCInteraction.waitForConversationWindow();
    ...
    public void talkToCook() {
        FremennikTrials.stage = "Talking to Cook";
        Utilities.walkToArea(COOK_AREA);
        Utilities.talkToNPC(COOK);
        NPCInteraction.waitForConversationWindow();
        NPCInteraction.handleConversation("Ask about what happened in the castle cellar");
        NPCInteraction.handleConversation();
    }

    public void talkToBob() {
        if (RSVarBit.get(532).getValue() == 1) {
            FremennikTrials.stage = "Talking to Bob";
            Utilities.walkToArea(BOB_AREA);
            Utilities.talkToNPC(BOB);
            NPCInteraction.waitForConversationWindow();
    ...
    public void talkToHans() {
        if (RSVarBit.get(532).getValue() == 1) {
            FremennikTrials.stage = "Going to Hans";
            Utilities.walkToArea(HANS_AREA);
            if (NPCs.find(HANS).length > 0) {
                FremennikTrials.stage = "Waiting for Hans...";
                Timing.waitCondition(() -> HANS_AREA.contains(NPCs.find(HANS)[0].getPosition()) && NPCs.find(HANS)[0].isOnScreen(), 45000);
            }
            FremennikTrials.stage = "Talking to Hans";
            Utilities.talkToNPC(HANS);
            NPCInteraction.waitForConversationWindow();
    ...
    public void talkToAereck() {
        if (RSVarBit.get(532).getValue() == 1) {
            FremennikTrials.stage = "Talking to Father Aereck";
            Utilities.walkToArea(CHURCH_AREA);
            Utilities.talkToNPC(FATHER_AERECK);
            NPCInteraction.waitForConversationWindow();
            NPCInteraction.handleConversation("Do you know what happened in the castle cellar?");
            NPCInteraction.handleConversation();
        }
    ...
    public void step3() {
        FremennikTrials.stage = "Reporting to Duke Horacio";
        Utilities.walkToArea(START_AREA);
        Utilities.talkToNPC(DUKE_HORACIO);
        NPCInteraction.waitForConversationWindow();
        NPCInteraction.handleConversation("Father Aereck says he saw something in the cellar", "Hans says he saw something in the cellar", "Bob says he saw something in the cellar");
        NPCInteraction.handleConversation();
    }

    public void step4() {
        FremennikTrials.stage = "Going to dig the tunnel";
        Utilities.walkToArea(CAVE_ENTRANCE);
        Utilities.useItemOnObject(STEEL_PICKAXE, 6898);
        NPCInteraction.waitForConversationWindow();
        NPCInteraction.handleConversation();
    }
    ...
    public void step5() {
        if (Inventory.find(BROACH).length < 1) {
            Utilities.useItemOnItem(UNLIT_CANDLE_LANTERN, TINDERBOX);
            FremennikTrials.stage = "Going to get Broach";
            if (CAVE_ENTRANCE.contains(Player.getPosition()) && !AFTER_TUNNEL_ENTRANCE.contains(Player.getPosition())) {
                Utilities.walkToArea(CAVE_ENTRANCE);
                Utilities.clickObject(HOLE, "Squeeze-through");
                Timing.waitCondition(() -> !CAVE_ENTRANCE.contains(Player.getPosition()), 15000);
            }
            if (AFTER_TUNNEL_ENTRANCE.contains(Player.getPosition())) {
                if (GroundItems.find(BROACH).length > 0) {
                    Utilities.blindWalkToTile(GroundItems.find(BROACH)[0].getPosition());
                    Constants.idle(3000, 6000);
                }
                Utilities.clickGroundItem(BROACH);
                Timing.waitCondition(() -> Inventory.find(BROACH).length > 0, 15000);
                leaveCave();
            }
        }
    }
    ...
    public void leaveCave() {
        Utilities.blindWalkToTile(new RSTile(3222, 9618, 0));
        Utilities.clickObject(6899, "Squeeze-through");
        Timing.waitCondition(() -> CAVE_ENTRANCE.contains(Player.getPosition()), 15000);
    }
    ...
    public void step7() {
        if (Inventory.find(BOOK).length < 1) {
            FremennikTrials.stage = "Going to Reldo";
            Utilities.walkToArea(RELDO_AREA);
            Utilities.talkToNPC(RELDO);
            NPCInteraction.waitForConversationWindow();
            NPCInteraction.handleConversation("Ask about the brooch.");
            NPCInteraction.handleConversation();
            Utilities.clickObject(6916, "Search");
            NPCInteraction.waitForConversationWindow();
            NPCInteraction.handleConversation();
        }
        if (Inventory.find(BOOK).length > 0) {
            AccurateMouse.click(Inventory.find(BOOK)[0], "Read");
            Timing.waitCondition(() -> Interfaces.get(183, 16) != null, 5000);
            if (Interfaces.get(183, 16) != null && RSVarBit.get(541).getValue() != 5) {
                Interfaces.get(183, 16).click();
                Constants.idle(150, 900);
                Interfaces.get(183, 16).click();
                Constants.idle(150, 900);
                Interfaces.get(183, 16).click();
                Constants.idle(150, 900);
    ...
    public void step8() {
        FremennikTrials.stage = "Going to Goblin Generals.";
        Utilities.walkToArea(GOBLIN_VILLAGE);
        Utilities.talkToNPC(669);
        NPCInteraction.waitForConversationWindow();
        NPCInteraction.handleConversation("Have you ever heard of the Dorgeshuun?", "It doesn't really matter", "Well either way they refused to fight", "Well I found a brooch underground...");
        NPCInteraction.handleConversation("Well why not show me both greetings?");
        NPCInteraction.handleConversation();
    }

    public void step9() {
        FremennikTrials.stage = "Going to Duke Horacio";
        Utilities.walkToArea(START_AREA);
        Utilities.talkToNPC(DUKE_HORACIO);
        NPCInteraction.waitForConversationWindow();
    ...
    public void step10() {
        Utilities.useItemOnItem(UNLIT_CANDLE_LANTERN, TINDERBOX);
        FremennikTrials.stage = "Going to Tunnel";
        ...
        if (AFTER_TUNNEL_ENTRANCE.contains(Player.getPosition())) {
            FremennikTrials.stage = "Navigating Tunnel";
            if (Inventory.find(Constants.STAMINA_POTION).length > 0) {
                AccurateMouse.click(Inventory.find(Constants.STAMINA_POTION)[0], "Drink");
            }
            Walking.walkPath(PATH_PT_1);
            Constants.idle(1000, 4000);
            Walking.walkPath(PATH_THROUGH_TUNNEL);
            Constants.idle(5000, 25000);
        }
        if (END_OF_MINE.contains(Player.getPosition())) {
            FremennikTrials.stage = "Talking to Mistag";
            Utilities.talkToNPC(MISTAG);
            NPCInteraction.waitForConversationWindow();
            NPCInteraction.handleConversation();
    ...
    public void step12() {
        if (Inventory.find(CHEST_KEY).length < 1 && Inventory.find(HAM_HOOD).length < 1) {
            FremennikTrials.stage = "Pickpocketing Sigmund";
            Utilities.walkToArea(START_AREA);
            if (NPCs.find(SIGMUND).length > 0) {
                Utilities.walkToTile(NPCs.find(SIGMUND)[0].getPosition());
                AccurateMouse.click(NPCs.find(SIGMUND)[0], "Pickpocket");
                Timing.waitCondition(() -> Inventory.find(CHEST_KEY).length > 0, 10000);
                Constants.idle(500, 2000);
            }
        }
        if (Inventory.find(CHEST_KEY).length > 0 && Inventory.find(HAM_HOOD).length < 1) {
            FremennikTrials.stage = "Opening Chest";
            Utilities.walkToTile(new RSTile(3209, 3216, 1));
            Utilities.clickObject(CLOSED_CHEST, "Open");
            NPCInteraction.waitForConversationWindow();
            NPCInteraction.handleConversation();
        }
    ...
    public void step13() {
        if (Inventory.find(HAM_HOOD).length > 0 && Inventory.find(SILVERWARE).length < 1) {
            if (!HIDE_OUT.contains(Player.getPosition())) {
                FremennikTrials.stage = "Going to Hideout";
                Utilities.walkToArea(ENTRANCE_TO_HIDEOUT);
                Utilities.clickObject(TRAPDOOR, "Pick-Lock");
                NPCInteraction.waitForConversationWindow();
                Utilities.clickObject(TRAPDOOR, "Climb-down");
                Timing.waitCondition(() -> HIDE_OUT.contains(Player.getPosition()), 10000);
            }
            if (HIDE_OUT.contains(Player.getPosition())) {
                FremennikTrials.stage = "Getting Silverware";
                Utilities.blindWalkToTile(new RSTile(3152, 9646));
                Utilities.clickObject(CRATE, "Search");
                NPCInteraction.waitForConversationWindow();
                NPCInteraction.handleConversation();
            }
        }
    ...
    public void step14() {
        FremennikTrials.stage = "Going to give treaty";
        if (!CAVE_ENTRANCE.contains(Player.getPosition()) && !END_OF_MINE.contains(Player.getPosition()) &&
                !AFTER_TUNNEL_ENTRANCE.contains(Player.getPosition())) {
            Utilities.walkToArea(CAVE_ENTRANCE);
            Utilities.clickObject(HOLE, "Squeeze-through");
        ...
        if (AFTER_TUNNEL_ENTRANCE.contains(Player.getPosition())) {
            if (NPCs.find(KAZGAR).length > 0) {
                Utilities.blindWalkToTile(NPCs.find(KAZGAR)[0].getPosition());
                AccurateMouse.click(NPCs.find(KAZGAR)[0], "Follow");
                Constants.idle(4000, 7000);
            }
        }
        if (END_OF_MINE.contains(Player.getPosition())) {
            FremennikTrials.stage = "Talking to Mistag";
            ...
            NPCInteraction.waitForConversationWindow();
            NPCInteraction.handleConversation();
            Constants.idle(1500, 4000);
            NPCInteraction.handleConversation();
            FremennikTrials.stage = "Finishing...";
            Constants.idle(1500, 4000);
            NPCInteraction.waitForConversationWindow();
            NPCInteraction.handleConversation();
            Utilities.cutScene();
    ...
    public void execute() {
        checkRequirements();
        FremennikTrials.currentQuest = "Lost Tribe";
        while (FremennikTrials.LOST_TRIBE) {
            General.sleep(100);
    ...
    public boolean validate() {
        return FremennikTrials.LOST_TRIBE;
    }
}
